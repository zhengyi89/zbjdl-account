						...
						<q:table queryService="queryService"
				            ...
				            <q:column title="操作" escapeHtml="false" width="20%">
				            	<c:if test="${status == 'ASSIGN'}">
				   			    	<a class="pink" onclick="window.location.href='${ctx}/company/accountbook/edit?id=${id}'">创建帐套</a>
				   			    	&nbsp;
				   			    </c:if>
				   			    <c:if test="${status != 'ASSIGN'}">
				   			    	<a class="pink" onclick="window.location.href='${ctx}/system/console?companyId=${id}&belongSystem=-9992002&_menuId=70&_firstMenuId=-2002'">记账</a>&nbsp;
				   			   		&nbsp;
				   			    </c:if>
							</q:column>
						</q:table>
